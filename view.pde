...
float d=300, b=QUARTER_PI, a=QUARTER_PI; // view parameters: distance, angles, q
...
void showAxes(float s) { // shows three orthogonal axes as red, green, blue blocks aligned with the local frame coordinates
  textSize(10);
  pushMatrix(); 
  pushMatrix(); fill(red);  scale(2*s,1,1); box(1); popMatrix();
  pushMatrix(); translate(s,1,1); text('X',0,0,0); popMatrix();
  pushMatrix(); fill(green);  scale(1,2*s,1); box(1); popMatrix();
  pushMatrix(); translate(1,s,1); text('Y',0,0,0); popMatrix();
  pushMatrix(); fill(blue);  scale(1,1,2*s); box(1); popMatrix();
  pushMatrix(); translate(1,1,s); text('Z',0,0,0); popMatrix();
  popMatrix();
  noStroke();noFill();
  }
